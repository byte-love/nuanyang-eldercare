...
import com.nuanyang.nursing.domain.RoomType;
import com.nuanyang.nursing.service.IRoomTypeService;
...
public class RoomTypeController extends BaseController
{
    ...
    {
        LambdaQueryWrapper<RoomType> queryWrapper = Wrappers.<RoomType>lambdaQuery()
                .eq(RoomType::getStatus, 1)
                .select(RoomType::getId, RoomType::getName);
        List<RoomType> list = roomTypeService.list(queryWrapper);
        return success(list);
    }
